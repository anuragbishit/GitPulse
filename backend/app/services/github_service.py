...
_ACTIVE_GITHUB_USERNAME: str | None = GITHUB_USERNAME
_ACTIVE_GITHUB_TOKEN: str | None = GITHUB_TOKEN


def set_active_github_credentials(username: str, token: str) -> None:
    """Use a user's GitHub credentials for the current in-process session.

    The default environment values remain the fallback, so the existing app keeps
    working as-is when no login override is active.
    """
    global _ACTIVE_GITHUB_USERNAME, _ACTIVE_GITHUB_TOKEN
    _ACTIVE_GITHUB_USERNAME = username.strip()
    _ACTIVE_GITHUB_TOKEN = token.strip()


def clear_active_github_credentials() -> None:
    global _ACTIVE_GITHUB_USERNAME, _ACTIVE_GITHUB_TOKEN
    _ACTIVE_GITHUB_USERNAME = GITHUB_USERNAME
    _ACTIVE_GITHUB_TOKEN = GITHUB_TOKEN


def get_active_github_context() -> tuple[str, str]:
    username = _ACTIVE_GITHUB_USERNAME or GITHUB_USERNAME
    token = _ACTIVE_GITHUB_TOKEN or GITHUB_TOKEN
    return username, token


def build_headers(token: str | None = None) -> dict:
    resolved_token = token or _ACTIVE_GITHUB_TOKEN or GITHUB_TOKEN
    return {
...
def _paginate(url: str, extra_params: dict = None, token: str | None = None) -> list[dict]:
    results, page = [], 1
    while True:
        params = {"per_page": 100, "page": page}
        if extra_params:
            params.update(extra_params)
        resp = requests.get(url, headers=build_headers(token), params=params, timeout=15)
...
def fetch_all_followers() -> list[dict]:
    ...
    return _paginate(f"https://api.github.com/users/{username}/followers")
...
def fetch_user_by_login(login: str) -> dict | None:
    ...
    resp = requests.get(f"https://api.github.com/users/{login}", headers=build_headers(), timeout=15)
...
def fetch_user_by_id(user_id: int) -> dict | None:
    ...
    resp = requests.get(f"https://api.github.com/user/{user_id}", headers=build_headers(), timeout=15)
    if resp.status_code == 404:
        return None
    resp.raise_for_status()
    return resp.json()
...
def fetch_my_profile() -> dict:
    resp = requests.get("https://api.github.com/user", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    p = resp.json()
...
def fetch_repo_languages(full_name: str) -> dict:
    resp = requests.get(f"https://api.github.com/repos/{full_name}/languages", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    return resp.json()


def fetch_repo_traffic_views(full_name: str) -> dict:
    resp = requests.get(f"https://api.github.com/repos/{full_name}/traffic/views", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    return resp.json()


def fetch_repo_traffic_clones(full_name: str) -> dict:
    resp = requests.get(f"https://api.github.com/repos/{full_name}/traffic/clones", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    return resp.json()


def fetch_repo_subscribers(full_name: str) -> int:
    resp = requests.get(f"https://api.github.com/repos/{full_name}", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    return resp.json().get("subscribers_count", 0)


def fetch_repo_referrers(full_name: str) -> list:
    resp = requests.get(f"https://api.github.com/repos/{full_name}/traffic/referrers", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    return resp.json() or []


def fetch_repo_popular_paths(full_name: str) -> list:
    resp = requests.get(f"https://api.github.com/repos/{full_name}/traffic/popular/paths", headers=build_headers(), timeout=15)
    resp.raise_for_status()
    return resp.json() or []
...
def search_users(query: str, per_page: int = 10) -> list[dict]:
    """Search public GitHub users by login or display name.

    This is intentionally a read-only helper for looking up other GitHub profiles
    without touching the authenticated user's sync data. It is safe to use in a
    search box or profile lookup flow as part of this app's analytics experience.
    """
    resp = requests.get(
