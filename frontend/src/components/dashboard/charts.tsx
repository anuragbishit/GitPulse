...
import type { ContributionYearSummary, FollowerGrowthPoint, StarPoint } from "@/lib/types";
...
export function FollowerGrowthChart({
  ...
  points: FollowerGrowthPoint[] | undefined;
  loading?: boolean;
}) {
  if (loading) return <Skeleton className="mx-6 mb-6 h-56" />;
  if (!points?.some((point) => point.followed || point.unfollowed)) {
    return <p className="px-6 pb-6 text-[13px] text-ink-3">No follower events in this period.</p>;
  }
    ...
    <div className="h-56 px-3 pb-4">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={points} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="growth-followed" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={ACCENT} stopOpacity={0.28} />
              <stop offset="100%" stopColor={ACCENT} stopOpacity={0} />
            </linearGradient>
          </defs>
          ...
          <XAxis
            dataKey="date"
            {...axisProps}
            tickFormatter={(v: string) => parseApiDate(v)?.toLocaleDateString("en-US", { month: "short", day: "numeric" }) ?? ""}
            minTickGap={28}
          />
          ...
          <RTooltip
            ...
            content={({ active, payload, label }) => {
              if (!active || !payload?.length) return null;
              const point = payload[0].payload as FollowerGrowthPoint;
              return (
                <TipBox
                  title={String(label)}
                  rows={[
                    { label: "New followers", value: comma(point.followed), color: ACCENT },
                    { label: "Unfollowed", value: comma(point.unfollowed), color: SERIES.issues },
                    { label: "Net change", value: comma(point.net) },
                  ]}
                />
              );
            }}
          />
          <Area type="monotone" dataKey="followed" name="New followers" stroke={ACCENT} fill="url(#growth-followed)" strokeWidth={2} />
          <Area type="monotone" dataKey="unfollowed" name="Unfollowed" stroke={SERIES.issues} fill="transparent" strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
