...
async function get<T>(path: string, signal?: AbortSignal): Promise<T> {
  const res = await fetch(`${API}${path}`, { signal, credentials: "include" });
  if (!res.ok) {
    throw new ApiError(
...
export const api = {
  ...
  userSearch: (q: string, per_page = 10, signal?: AbortSignal) =>
    get<import("./types").GitHubUserSearchResponse>(`/users/search${query({ q, per_page })}`, signal),
  ...
  followerGrowth: (days = 90, signal?: AbortSignal) =>
    get<FollowerGrowth>(`/followers/growth${query({ days })}`, signal),
  ...
  repoAnalysis: (name: string, signal?: AbortSignal) =>
    get<RepoAnalysis>(`/repos/${encodeURIComponent(name)}/analysis`, signal),
  ...
  syncRuns: (limit = 10, signal?: AbortSignal) =>
    get<SyncRun[]>(`/sync-runs${query({ limit })}`, signal),
  ...
  logout: async (): Promise<void> => {
    ...
    if (!res.ok) throw new ApiError(`Logout failed (${res.status})`, res.status);
  },

  aiInsights: (signal?: AbortSignal) => get<import("./types").AiInsights>("/analytics/ai-insights", signal),
  audienceQuality: (signal?: AbortSignal) => get<AudienceQuality>("/analytics/quality", signal),
  repositoryOpportunityRadar: (signal?: AbortSignal) => get<RepositoryOpportunityRadar>("/analytics/opportunities", signal),
  growthForecast: (signal?: AbortSignal) => get<GrowthForecast>("/analytics/forecast", signal),
  smartAlerts: (signal?: AbortSignal) => get<SmartAlerts>("/analytics/alerts", signal),
  exportUrl: (format: "csv" | "json") => `${API}/analytics/export?format=${format}`,
  badgeUrl: () => `${API}/analytics/badge/followers`,
};
